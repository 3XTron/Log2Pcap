...
V0.2: April 2015, Enzo Version, dedicated to @Spankito.
 ...
 - Process evolution.
V0.3: Modernized to Python3
 TODO:
 - Resume the process.
"""

__author__ = "Joaquin Moreno Garijo @moxilo, modernized by GPT"
__version__ = "0.3"
...
class ipv4Tcp:
    def __init__(self, serverIp, serverPort):
        ...
        self.srcPort = 30003  # Changed to int for consistency

    def handshake(self, srcIp, tm):
        pkts = []
        # Move this to constructor to be faster. (Some app will complain)
        self.srcPort = random.randint(1024, 65535)  # Ensure srcPort is within valid range
        # Random seq
    ...
    def queryResponse(self, srcIp, seqN, ackN, query, response, tm):
        pkts = []
        # Query
        pkt = IP(dst=self.dstIp, src=srcIp) / TCP(dport=int(self.dstPort), sport=int(self.srcPort), flags="A", seq=seqN, ack=ackN) / query.encode()  # Encode to bytes
        pkt.time = tm
        pkts.append(pkt)
        # Seq incremental
        if len(query) == 0:
            ...
            seqN = seqN + len(query)
        # Response
        pkt = IP(dst=srcIp, src=self.dstIp) / TCP(dport=int(self.srcPort), sport=int(self.dstPort), flags="A", seq=ackN, ack=seqN) / response.encode()  # Encode to bytes
        pkt.time = tm
        pkts.append(pkt)
        # Acq incremental
        if len(response) == 0:
    ...
    def sendData(self, srcIp, seqN, ackN, data, tm):
        # Query
        pkt = (IP(dst=self.dstIp, src=srcIp) / TCP(dport=int(self.dstPort), sport=int(self.srcPort), flags="A", seq=seqN, ack=ackN) / data.encode())  # Encode to bytes
        pkt.time = tm
        # Seq incremental
        if len(data) == 0:  # Corrected variable name from 'query' to 'data'
            seqN = seqN + 1
        else:
            seqN = seqN + len(data)
...
class log2pcap:
    def __init__(self, logFile, outputFileName, template, serverIp, serverPort, dumpToFile=1):
        ...
        try:
            with open(logFile, 'rb') as f:  # Use 'with' for automatic file closing
                pass  # Just test if the file can be opened
        except Exception as e:
            sys.stderr.write("\n[Error] Log File \"%s\" not found: %s, exit...\n\n" % (logFile, str(e)))
            sys.exit(1)
        self.pcapFileName = outputFileName
        templates = ['apache', 'webseal', 'nginx', 'iis', 'iis-w3c', 'tomcat']
        if template not in templates:
            raise ValueError(f"Template '{template}' is not a valid template.")
        self.template = template
        self.proto = protocol(serverIp, serverPort)
        try:
            self.dumpToFile = int(dumpToFile)
        except ValueError:
            raise ValueError(f"Invalid value for dumpToFile: {dumpToFile}.  Must be an integer.")
    ...
    def processFile(self, typeLog):
        ...
        try:
            ...
            sys.stderr.write("\n[Error] Log File \"%s\" not found: %s, exit...\n\n" % (self.logFileName, str(e)))
            sys.exit(1)
        try:
            pcapfile = PcapWriter(self.pcapFileName, append=True)
        except Exception as e:
            sys.stderr.write("\n[Error] Create PCAP file \"%s\" not allowed: %s, exit...\n\n" % (self.pcapFileName, str(e)))
            sys.exit(1)
        ...
        for log in logFile:
            try:
                result = methodLogLine(log)
                if result:
                    (srcIp, url, method, protocol, useragent, referer, param, retCode, tm) = result
                    traffic.extend(self.proto.http(srcIp, url, method, protocol, useragent, referer, param, retCode, tm))
                    cont = cont + 1
            except Exception as e:
                sys.stderr.write(f"Error processing log line: {e}\n")
                continue
